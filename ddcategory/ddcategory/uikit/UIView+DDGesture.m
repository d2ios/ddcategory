...
@implementation DDGestureCallbackValues
...
@implementation UIView (DDGesture)
...
-(void)addTapGestureRecognizer:(void(^)(UITapGestureRecognizer* recognizer, NSString* gestureId))tapCallback  tapGestureId:(NSString*)tapGestureId  numberOfTapsRequired:(NSUInteger)numberOfTapsRequired  numberOfTouchesRequired:(NSUInteger)numberOfTouchesRequired
{
    ...
    DDGestureCallbackValues *v = [DDGestureCallbackValues new];
    v.gesture = tg;
...
{
    DDGestureCallbackValues *v = [self.gestures objectForKey:tapGestureId];
    if (v != nil) {
...
-(void)removeAllTapGestures
{
    ...
    for (DDGestureCallbackValues *v in arr) {
...
{
    DDGestureCallbackValues *v = [self.gestureKeysHash objectForKey:[NSString stringWithFormat:@"%lu", (unsigned long)recognizer.hash]];
...
{
    DDGestureCallbackValues *v = [self.gestures objectForKey:pinchGestureId];
    if (v != nil) {
...
{
    DDGestureCallbackValues *v = [self.gestures objectForKey:panGestureId];
    if (v != nil) {
...
{
    DDGestureCallbackValues *v = [self.gestures objectForKey:swipeGestureId];
    if (v != nil) {
...
{
    DDGestureCallbackValues *v = [self.gestures objectForKey:rotationGestureId];
    if (v != nil) {
...
{
    DDGestureCallbackValues *v = [self.gestures objectForKey:longPressGestureId];
    if (v != nil) {
