...
#import "NSDate+DDExts.h"

#import <time.h>
#import <xlocale.h>

@implementation NSDate (DDExts)
...
{
    NSDate *date = self;
    NSInteger m = abs(years);
    NSInteger flag = years/m;
    
    for (int i = 0; i < m; i++) {
        NSInteger year = [date daysOfYear];
        date = [date dateByAddingDays:flag * year];
    }
    return date;
}
